    ...
    if (skait == 1) {
        ...
        for (int i : sarasas) {
            ...
            for (int j = 0; j < testsk; j++) {
                ...
                Timer1 t3;
                surusiuoti(grupe, vargsiukai, pagalRusiuoti);
                double z3 = t3.elapsed();
                laikas3 += z3;

                Timer4 t4;
                isvedimas(vargsiukai, "vargsiukai_" + to_string(i) + "." + to_string(j) + ".txt");
                double z4 = t4.elapsed();
                laikas4 += z4;

                Timer5 t5;
                isvedimas(grupe, "galvociai_" + to_string(i) + "." + to_string(j) + ".txt");
            ...
            isvedimasLaiko(laikas1, laikas2, laikas3, laikas4, laikas5, testsk, i);
        }
    }
    system("pause");
    return 0;
